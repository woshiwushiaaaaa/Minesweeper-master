...
namespace Minesweeper
{
    public partial class Form_Main : Form
    {
            ...
            for (int i = 1; i <= nWidth; i++)    // 绘制行
            {
                for (int j = 1; j <= nHeight; j++)   // 绘制列
                {
                    ...
                    else if(pState[i, j] == 1)  // 点开
                    {
                        ...
                        if(pMine[i, j] == -1)
                        {
                            g.DrawImage(Properties.Resources.Mine, nOffsetX + 28 * (i - 1)+1 , nOffsetY + 28 * (j - 1)+1 );   // 绘制地雷
                        }
                    }
                }
            }
        ...
        private void UpdateSize()
        {
            ...
            int nAdditionY =TableLayoutPanel_Main.Height;  // 包含了菜单栏以及下方显示信息栏的高度————————————————————————————————————
            this.Width = 12 + 28 * nWidth + nOffsetX;   // 设置窗口高度，34为每个雷区的高度，12为上下总空隙（6px+6px），再加上偏移量
            this.Height = 12 + 28 * nHeight + nAdditionY + nOffsetY;    // 设置窗口宽度，同理
        ...
        private void newGameNToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //==============================================
            pMine[2, 1] = pMine[4, 1] = pMine[5, 1] = -1;
            pMine[1,2] = pMine[10,2] = pMine[18,2] = -1;
            pMine[9,3] = pMine[18,3] = -1;
            pMine[13,4] = pMine[19,4] = -1;
            pMine[3,5] = pMine[5,5] = pMine[14,5] = -1;
            pMine[4,6] = pMine[7,6] = pMine[15,6] = -1;
            pMine[5,7]= -1;
            pMine[18,8]= -1;
            pMine[4,9] = pMine[13,9] = -1;
            pMine[2,10] = pMine[3,10] = pMine[12,10] = -1;
            pMine[2,11] = pMine[13,11] = -1;
            pMine[2,12] = pMine[4,12] = pMine[6,12] = pMine[10,12] = pMine[12,12] = pMine[16,12] = pMine[17,12] = pMine[18,12] = -1;
            pMine[1,13] = pMine[7,13] = pMine[12,13] = pMine[14,13] = pMine[16,13] = pMine[19,13] = -1;
            pMine[3,15] = pMine[13,15] = pMine[14,15] = pMine[15,15] = pMine[12,15] = pMine[17,15] = -1;
            pMine[12,16] = pMine[16,16] = -1;
            pMine[2,17] = pMine[3,17] = pMine[11,17] = -1;
            pMine[2,18] = pMine[5,18] = pMine[11,18] = -1;
            pMine[2,19] = pMine[10,19] = -1;
            pMine[2,20] = pMine[5,20] = pMine[11,20] = -1;
            pMine[3,21] = pMine[18,21] = -1;
            pMine[2,22] = pMine[10,22] = -1;
            pMine[4,23] = pMine[10,23] = pMine[11,23] = -1;
            pMine[3,24] = pMine[14,24] = pMine[17, 24] = -1;
            pMine[6,26] = -1;
            //=======================================================
        ...
        private void Form_Main_MouseMove(object sender, MouseEventArgs e)
        {
            if(e.X < 6 || e.X > 6 + nWidth * 28 || 
                e.Y < 6  || 
                e.Y > 6  + nHeight * 28) // 不在地雷区域——————————————————————————————————————————————————————
            {
                MouseFocus.X = 0; MouseFocus.Y = 0;
            }
            else
            {
                int x = (e.X - 6) / 28 + 1; // 获取x位置
                int y = (e.Y - 6) / 28 + 1; // 获取y位置————————————————————————————————————————————————————————
